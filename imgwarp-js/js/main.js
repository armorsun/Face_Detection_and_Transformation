...
$(document).ready(function(){  
  ...
  $('.redraw').change(function () {
	...
	$("#smile").on("change", function() {
		//alert($(this).val()); 
		var smile_val = $(this).val() / 1000;
		warper.adjust(smile_val, 0.1 + (1.0 - smile_val));
	});
    ...
    if (e.dataTransfer.files.length > 0) {
      ...
      reader.onload = function (event) {
        ...
        var img = render(event.target.result, function (imageData) {
		  var imgData = imageData[0];
		  var scale = imageData[1];
          if (warper) {
            delete warper;
          }
          warper = new ImgWarper.PointDefiner(canvas, img, imgData, scale);
        });
      };
...
function render(src, callback){
  ...
  image.onload = function(){
    ...
    callback([ctx.getImageData(0, 0, image.width, image.height), scale]);
  };
  image.src = src;
  return image;
}
